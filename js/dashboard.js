...
(function () {
  ...
  function handleResume() {
    ...
    if (session.type === "ruby-learning") {
    ...
    } else if (session.type === "rails-principles") {
      // Rails基礎原則モードの続き
      if (window.RailsPrinciples && window.RailsPrinciples.resumeAt) {
        window.RailsPrinciples.resumeAt(session.chapterId, session.stepIndex);
      }
      ...
      const btn = document.querySelector(
        `.mode-btn[data-mode="${session.mode}"]`,
      );
      if (btn) btn.click();
    }
  }
  ...
  function renderResumeButton() {
    ...
    let label = "前回の続きから再開";
    if (
      session.type === "ruby-learning" &&
      typeof RUBY_LEARNING_DATA !== "undefined"
    ) {
      ...
      session.type === "git-learning" &&
      typeof GIT_LEARNING_DATA !== "undefined"
    ) {
      ...
      session.type === "rails-learning" &&
      typeof RAILS_LEARNING_DATA !== "undefined"
    ) {
      ...
      session.type === "rails-principles" &&
      typeof RAILS_PRINCIPLES_DATA !== "undefined"
    ) {
      const ch = RAILS_PRINCIPLES_DATA[session.chapterId];
      if (ch) {
        const step = ch.steps[session.stepIndex];
        label = `続きから再開: 原則 Ch.${session.chapterId} - ${step ? step.title : ""}`;
      }
  ...
  function renderCalendar() {
    ...
    days.forEach((d) => {
      html += `<div class="dash-cal-cell dash-cal-weekday">${d}</div>`;
    });
    html += "</div>";
  ...
  function renderProgress() {
    ...
    const pct =
      totalQ > 0 ? Math.min(Math.round((sessions / totalQ) * 100), 100) : 0;
