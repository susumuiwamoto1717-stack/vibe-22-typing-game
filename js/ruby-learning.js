...
(function () {
  ...
  function showChapterScreen() {
    ...
    RUBY_LEARNING_CHAPTERS.forEach((ch) => {
      ...
      const stepCount = RUBY_LEARNING_DATA[ch.id]
        ? RUBY_LEARNING_DATA[ch.id].steps.length
        : 0;
      btn.innerHTML = `
  ...
  function attachRLInputHandlers() {
    ...
    els.rlImeInput.addEventListener(
      "compositionstart",
      handleRLCompositionStart,
    );
    ...
    els.rlImeInput._blurHandler = () => {
      if (
        document
          .getElementById("screen-ruby-typing")
  ...
  function detachRLInputHandlers() {
    ...
    els.rlImeInput.removeEventListener(
      "compositionstart",
      handleRLCompositionStart,
    );
    els.rlImeInput.removeEventListener(
      "compositionend",
      handleRLCompositionEnd,
    );
  ...
  function startRLTimer() {
    ...
    rlState.timerInterval = setInterval(() => {
      rlState.elapsedSeconds = Math.floor(
        (Date.now() - rlState.startTime) / 1000,
      );
      const min = String(Math.floor(rlState.elapsedSeconds / 60)).padStart(
        2,
        "0",
      );
  ...
  function showRLComplete(noCode) {
    ...
    if (noCode) {
      ...
      const elapsedMin = rlState.startTime
        ? (Date.now() - rlState.startTime) / 60000
        : 0;
      const wpm =
        elapsedMin > 0.01
          ? Math.round(rlState.totalCorrect / 5 / elapsedMin)
          : 0;
      const accuracy =
        rlState.totalChars > 0
          ? Math.floor((rlState.totalCorrect / rlState.totalChars) * 100)
          : 100;
      const totalSec = rlState.startTime
        ? Math.floor((Date.now() - rlState.startTime) / 1000)
        : 0;
  ...
  function initRubyLearning() {
    ...
    els.passwordBack.addEventListener("click", () => {
      ...
      document
        .getElementById("screen-ruby-password")
    ...
    const btnCourseRails = document.getElementById("btn-course-rails");
    if (btnCourseRails) {
      btnCourseRails.addEventListener("click", () => {
        if (window.RailsLearning && window.RailsLearning.showChapters) {
          window.RailsLearning.showChapters();
        }
      });
    }
    ...
    if (courseBack) {
      courseBack.addEventListener("click", () => {
        document.getElementById("screen-select").classList.add("active");
        document
          .getElementById("screen-runteq-courses")
          .classList.remove("active");
      });
    }
