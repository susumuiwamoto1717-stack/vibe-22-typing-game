...
(function () {
  ...
  function initKeyboards() {
    keyboardReady = new KeyboardRenderer('keyboard-ready', state.keyboardLayout);
    keyboardTyping = new KeyboardRenderer('keyboard-typing', state.keyboardLayout);
  ...
  function handleKeyDown(e) {
    ...
    if (e.key === 'Shift' || e.key === 'Control' || e.key === 'Alt' || e.key === 'Meta' ||
        ...
        e.key === 'Enter' || e.key === 'Delete' || e.key === 'Home' || e.key === 'End' ||
  ...
  function finishTyping() {
    ...
    const wpm = elapsedMin > 0.01 ? Math.round(state.totalCorrect / 5 / elapsedMin) : 0;
    const accuracy = state.totalChars > 0 ? Math.floor((state.totalCorrect / state.totalChars) * 100) : 100;
    ...
    // 時間表示
    const totalSec = state.startTime ? Math.floor((Date.now() - state.startTime) / 1000) : 0;
